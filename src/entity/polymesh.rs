...
impl Polygon {
    ...
    pub fn new(vertices: Vec<usize>, vertex_list: &[Vec3]) -> Option<Polygon> {
        ...
        todo!("Check if the vertices are coplanar.");
...
impl TriMesh {
    ...
    ///     &vertex_list,
    ///     &[Triangle::new([0, 2, 3], &vertex_list)]
    /// ).unwrap();
    ///
...
impl PolyMesh {
    ...
    pub fn from(vertices: &[Vec3], polygons: &[Polygon]) -> Option<Self> {
        if polygons.iter().flat_map(|f| &f.vertices).any(|&f| f >= vertices.len()) {
            return None; // Invalid vertices
        }
        todo!("Check if polygons are coplanar");
        // Some(PolyMesh {
        //    vertices: vertices.to_vec(),
